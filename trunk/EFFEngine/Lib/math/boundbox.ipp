...
INLINE BoundingBox::BoundingBox( const Vector3 & min, const Vector3 & max ) :
	...
	max_( max )
{
	// intentionally not initialising outcodes 'coz other constructor doesn't
}
...
{
	const Vector3 & min1 = this->minBounds();
	const Vector3 & max1 = this->maxBounds();

	const Vector3 & min2 = box.minBounds();
	const Vector3 & max2 = box.maxBounds();
