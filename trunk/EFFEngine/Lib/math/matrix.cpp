...
#else
struct __declspec(align(16)) afloat16
{
	float values[16];
};

static afloat16 s_identityMatrix =
{
	1.f, 0.f, 0.f, 0.f,
	0.f, 1.f, 0.f, 0.f,
	0.f, 0.f, 1.f, 0.f,
...
Matrix & Matrix::identity = *reinterpret_cast<Matrix *>( &s_identityMatrix );
 ...
 */
void Matrix::invert( const Matrix& matrix )
{
	float determinant = matrix.getDeterminant();

	MF_ASSERT(determinant != 0.f);
