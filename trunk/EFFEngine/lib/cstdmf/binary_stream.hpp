...
/**
 * @internal
 * This was initially PACKET_MAX_SIZE (for not obvious reason),
 * so was brought into binary_stream.hpp when it was merged into
 * cstdmf.
 */
...
public:
	...
	void appendString( const char * data, int length );
	...
	void writeStringLength( int len );
};
...
public:
	 ...
	 */
	virtual void * retrieve( int nBytes ) = 0;
...
{
	memcpy( this->reserve( size ), pBlob, size );
}
...
inline BinaryOStream & operator<<( BinaryOStream & b, const std::vector<T,A> & data)
{
...
/** This method puts the string length into the stream.
 *
 *	@param	len	The string length.
 */
inline void BinaryOStream::writeStringLength( int len )
{
	if (len >= 255)
	{
		(*this) << (uint8)0xff;
		BW_PACK3( (char*)this->reserve( 3 ), len );
	}
	else
	{
		(*this) << (uint8)len;
...
/**
 *	This method gets the string length from the stream.
 *
 *	@return	The string length.
 */
inline int BinaryIStream::readStringLength()
{
	int	len = *(uint8*)this->retrieve( 1 );

	if (len == 0xFF)
	{	// line below is very slightly dodgy ('tho safe here)
		len = BW_UNPACK3( (const char*)this->retrieve( 3 ) );
