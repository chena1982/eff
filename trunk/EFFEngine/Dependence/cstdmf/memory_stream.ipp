 ...
 */
inline void * MemoryOStream::retrieve( int nBytes )
{
	...
	IF_NOT_MF_ASSERT_DEV( pRead_ + nBytes <= pCurr_ )
	{
		...
		// pretty dire error
		return nBytes <= PACKET_MAX_SIZE ? errBuf : NULL;
	}
...
inline void *MemoryIStream::retrieve( int nBytes )
{
	char * pOldRead = pCurr_;
