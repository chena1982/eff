...
inline BinaryOStream& operator<<( BinaryOStream &out, bw_netlong x )
{
	...
	*(long*)out.reserve( sizeof( x ) ) = BW_HTONL( x.l );
	return out;
}
...
inline BinaryOStream& operator<<( BinaryOStream &out, bw_netlonglong x )
{
	...
	*(long long*)out.reserve( sizeof( x ) ) = BW_HTONLL( x.ll );
	return out;
}
...
inline BinaryOStream& operator<<( BinaryOStream &out, int x )
{
	bw_netlong n;
	n.i = x;
	return out << n;
...
{
	MF_ASSERT( sizeof( bw_netlong ) == 4 );
	x.l = BW_NTOHL( *(long*)in.retrieve( sizeof( x ) ) );
	return in;
}
...
{
	MF_ASSERT( sizeof( bw_netlonglong ) == 8 );
	x.ll = BW_NTOHLL( *(long long*)in.retrieve( sizeof( x ) ) );
	return in;
}
...
inline BinaryIStream& operator>>( BinaryIStream &in, int &x )
{
	bw_netlong n;
	in >> n;
	x = n.i;
...
inline BinaryIStream& operator>>( BinaryIStream &in, void* &x )
{
	bw_netlong n;
	in >> n;
	x = n.p;
