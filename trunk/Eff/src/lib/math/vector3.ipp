 ...
 *
 *	@note <i>i</i> must be either 0, 1 or 2.
 */
INLINE
float& Vector3::operator[]( int i )
{
#ifndef __BORLANDC__
	MF_ASSERT_DEBUG( i >= 0 && i < 3 );
#endif
...
#if 0
/**
 *	This function creates a vector from an input pitch and yaw. The z-axis is
 *	considered as have 0 pitch and 0 yaw.
 *
 *	@param pitchInRadians	The angle in radians that the result vector is
 *							rotated from the XZ plane.
 *	@param yawInRadians		The angle in radians that the result vector is
 *							rotated around the Y axis. A positive value rotates
...
Vector3 pitchYawToVector3( float pitchInRadians, float yawInRadians )
{
	...
	return Vector3(cosPitch * sinYaw, sinPitch, cosPitch * cosYaw);
}
#endif
