	...
	mPopupMenuWidgets(nullptr)
{
	...
	//tools::SettingsManager::getInstance().eventSettingsChanged.connect(this, &MainMenuControl::notifySettingsChanged);
...
{
	EditorWidgets::getInstance().eventChangeWidgets -= MyGUI::newDelegate(this, &MainMenuControl::notifyChangeWidgets);
	//tools::SettingsManager::getInstance().eventSettingsChanged.disconnect(this);
}
...
std::string MainMenuControl::getDescriptionString(MyGUI::Widget* _widget, bool _print_name, bool _print_type, bool _print_skin)
{
	...
	tools::addUserTag("WidgetName", _print_name ? widgetContainer->getName() : "");
	tools::addUserTag("WidgetType", _print_type ? _widget->getTypeName() : "");
	tools::addUserTag("WidgetSkin", _print_skin ? widgetContainer->getSkin() : "");

	tools::addUserTag("FormatWidgetName", (_print_name && !widgetContainer->getName().empty()) ? "#{PatternWidgetName}" : "");
	tools::addUserTag("FormatWidgetType", _print_type ? "#{PatternWidgetType}" : "");
	tools::addUserTag("FormatWidgetSkin", _print_skin ? "#{PatternWidgetSkin}" : "");

	return tools::replaceTags("MenuItemWidgetInfo");
}
